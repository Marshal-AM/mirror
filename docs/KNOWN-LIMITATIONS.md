...
| **Scoring FCE TEE-to-TEE channel** | Live | Matching `GET /internal/outcome-log` on `127.0.0.1:7702` gated by `TEE_INTERNAL_TOKEN`. AI FCE fetches that URL plus optional Vercel `/api/outcomes`. Production `SCORE_V1` does not send synthetic fixtures. New leads stay at 0 until a vault fill; the VM fill-worker then `updateScore`s (AI TEE or local formula). Canaries may still pass `fixture`. |
