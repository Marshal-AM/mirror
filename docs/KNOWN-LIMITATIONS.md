...
| **Scoring FCE TEE-to-TEE channel** | Live | Matching `GET /internal/outcome-log` on `127.0.0.1:7702` gated by `TEE_INTERNAL_TOKEN`. Fill-worker stores real `pnlBps` (FTSO quote vs `amountOut`, plus FXRP mark-to-market since the previous fill) and scores the **full** history. Mock SparkDEX fills at mid, so execution PnL is ~0; Sharpe moves from interval FTSO returns (need ≥2 fills). Canaries may still pass `fixture`. |
